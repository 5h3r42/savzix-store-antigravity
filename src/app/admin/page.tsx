        ...
        <div className="lg:col-span-2 bg-card border border-border rounded-3xl p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-2">
                <span className="px-3 py-1 rounded-full bg-muted text-xs font-medium cursor-pointer hover:bg-primary hover:text-primary-foreground transition-colors">Daily</span>
                <span className="px-3 py-1 rounded-full bg-primary text-primary-foreground text-xs font-medium cursor-pointer">Weekly</span>
                <span className="px-3 py-1 rounded-full bg-muted text-xs font-medium cursor-pointer hover:bg-primary hover:text-primary-foreground transition-colors">Monthly</span>
            </div>
          </div>
          
          <div className="h-[300px] w-full flex items-end justify-between gap-2 px-2">
              {[40, 65, 45, 80, 55, 90, 70, 85, 60, 75, 50, 95].map((h, i) => (
                  <div key={i} className="group relative w-full h-full bg-muted/20 rounded-t-xl hover:bg-primary/10 transition-colors flex items-end overflow-hidden">
                      <div 
                        style={{ height: `${h}%` }} 
          ...
          <div className="overflow-x-auto">
              <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-border">
                      {[
                          { id: "#ORD-7752", customer: "Elena Fisher", date: "Just now", status: "Processing", total: "$240.00" },
                          { id: "#ORD-7751", customer: "Nathan Drake", date: "2 min ago", status: "Completed", total: "$120.00" },
                          { id: "#ORD-7750", customer: "Chloe Frazer", date: "15 min ago", status: "Shipped", total: "$85.00" },
                          { id: "#ORD-7749", customer: "Victor Sullivan", date: "1 hour ago", status: "Completed", total: "$310.00" },
                          { id: "#ORD-7748", customer: "Sam Drake", date: "3 hours ago", status: "Pending", total: "$55.00" },
                      ].map((order, i) => (
...
function StatusBadge({ status }: { status: string }) {
    ...
    return (
        <span className={`px-3 py-1 rounded-full text-xs font-medium border ${styles[status] || "bg-gray-500/10 text-gray-500"}`}>
            {status}
        </span>
    );
}
